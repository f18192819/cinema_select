...
热度来源为已售、已购票和已预订座位。系统先用 `Map` 为热源去重，再按距离分段累计影响，最后只用座位外圈的颜色和微弱发光显示热度，避免覆盖座位实际状态。购买订单会额外写入 `purchasedAt`；一周视图展示含今天在内的最近 7 天，不显示未来日期。每日座位状态使用稳定随机生成，真实订单会覆盖对应日期的模拟状态；Canvas 填充和热度计算共用同一份每日状态，确保已售座位必定参与热度扩散。选座页的“显示热度·变化”逐日复用相同的距离扩散结果计算区域均热，并可点击某日让同一张 Canvas 预览对应热度边框。
